...
export const dataUser = {
  me: async () => {
    try {
      return await authUser.profil();
    } catch (error) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: getErrorMessage(error, 'User Tidak Ditemukan. Please try again.'),
      });
  ...
  updateData: async (data: any) => {
    try {
      ...
      const response = await authUser.updateProfil(formData);
      Swal.fire({
        icon: 'success',
...
export const authentication = {
  register: async (data: Register) => {
    try {
      ...
      throw new Error(
        getErrorMessage(error, 'Register failed. Please try again.')
      );
    }
  },
  login: async (data: Login) => {
    try {
      ...
      useAuthStore
        .getState()
        .setAuth({ accessToken: at, refreshToken: rt, role: role, id });
      return role;
    } catch (error) {
      Swal.fire({
        icon: 'error',
...
export const dataMember = {
  ...
  loadImage: async (props: any, type: any) => {
    type ImageItem = {
      index: number;
      src: string;
    };
    if (props) {
      const allowedFilePaths = type ?? {};
      const listImage: ImageItem[] = []; 

      await Promise.all(
        props.map(async (value: any, index: number) => {
          if (
            allowedFilePaths[value.filename as keyof typeof allowedFilePaths]
          ) {
            const src = await dataMember.getFile(value.filePath);
            listImage.push({ index, src: src || '' });
            return;
          }
        })
      );
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (value instanceof File) {
          ...
          console.warn(
            `Skipping field ${typedKey}, unexpected object format:`,
            value
          );
